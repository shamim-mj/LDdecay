...
#' @param marker_col Character. Name of the column containing unique variant identifiers in the `map` file. Default is `"SNP"`.
#' @param chr_col Character. Name of the column containing chromosome assignments in the `map` file. Default is `"Chromosome"`.
...
#' @param gene_name Character. Title label or text indicator assigned to the structural gene tracks matching `gene_pattern` in plot legends. Default is `"ABC/PDR Transport"`. Replace with gene function for regex to find related gene e.g. cytochrome p450|cytochrome|p450 
